...
docker_process_init_files() {
	...
	mysql=( mysql --defaults-extra-file=/etc/mysql/conf.d/docker.cnf )
	...
	for f; do
		case "$f" in
			...
			*.sql.xz)  mysql_note "$0: running $f"; xzcat "$f" | "${mysql[@]}"; echo ;;
			*.sql.zst) mysql_note "$0: running $f"; zstd -dc "$f" | "${mysql[@]}"; echo ;;
...
docker_temp_server_start() {
	if [ "${MYSQL_MAJOR}" = '5.7' ]; then
		...
		# For 5.7+ the server is ready for use as soon as startup completes
...
docker_init_database_dir() {
	...
	if [ "$MYSQL_MAJOR" = '5.7' ]; then
		...
		mysqld --initialize-insecure --default-time-zone=SYSTEM --log-error=mysql.err
	fi
...
#    ie: docker_process_sql --database=mydb <<<'INSERT INTO users (name) VALUES ("foo")'
...
docker_setup_db() {
	...
	local passwordSet=
	if [ "$MYSQL_MAJOR" = '5.7' ]; then
		# In 5.7, the root password can be set in the initial SQL statement
		passwordSet="SET PASSWORD FOR 'root'@'localhost'=PASSWORD('${MYSQL_ROOT_PASSWORD}') ;"
	else
		# In 8.0+, the root password must be set after the server is started
...
# else can be done (only supported for 5.7+)
